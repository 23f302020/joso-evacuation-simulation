...
| **E3** | 床効果判定・主指標の確定 | Claude(判断) | E2 | ✅ 完了（主指標ヌル・正本値への是正・E3本文確定。2026-07-20に生成済み図4-8-1 ECDFを本文へ接続し、「二峰性を示す図」を「run別完了率と初期条件感応性」へ改称） |
...
| **V4** | テスト結果記録（P3-IMPL-11） | Claude | V3 | ✅ 完了（2026-07-20・`04_プログラム/テスト結果/phase3/テスト結果_phase3.md`・399行・個別記録25本を統合〔B系〜段4・S系・V系〕。Phase 2テンプレ準拠7節構成＋アーク別詳細。撤回値/teleport警告/AC7廃止/demographic解釈禁止を限界節に収録・X-5遵守） |
...
| **DOC-FIG** | Phase 1〜3図表フロー同期・論文用結果図 | Codex | E3,V4 | △ **研究結果素材は充足・卒論編集のみ残**（2026-07-20再監査）。図4-8-1 ECDFは作成・E3接続済み、worst-off記述CSVも生成済み。残＝図4-7-1・図5-4-1・図5-5-1の静的画像と表5-6-1で、P2-DOC-2（年末）に実施。確定RQでは渋滞/worst-offの新規A/B判定図は不要 |
| **AUD-0** | 撤回コピー隔離＋正本manifest作成 | Claude+Sonnet | — | ✅ 完了（2026-07-20・旧Decision 128成果物をretractedへ一括隔離、引用禁止READMEと指標値を含まない正本manifestを作成、旧パス消失・4 SHA一致を確認） |
| **AUD-1** | RQ一本化（固定ルート・方向中立・指標階層明文化） | Sonnet | — | ✅ 完了（2026-07-20・固定ルート、方向中立RQ、事前仮説注記、正式判定/記述報告/従・限界の階層を同期。禁止表現候補0件を確認） |
| **AUD-2** | regime bimodality用語統一（現状文書のみ） | Sonnet | — | ✅ 完了（2026-07-20・現状4文書を「強い初期条件感応性／複数regimeの存在を示唆」へ統一、断定語0件・観測事実保持を確認。決定107は前方ポインタのみ追記） |
| **AUD-3** | worst-off記述統計CSV作成 | Claude+Sonnet | AUD-1 | ✅ 完了（2026-07-20・既存8runのvehicle logからA3/B5記述統計CSVを生成。車単位/到着者条件付き/B側バス乗客非含有/診断のみを明記、判定・符号列0、テスト2件成功、追加run/seedなし） |
| **AUD-4** | 限界節固定文言4点の追記 | Sonnet | AUD-1,AUD-3 | ✅ 完了（2026-07-20・L1〜L4を単一正本へ固定し、RQ/E3/V3/研究結果が同一アンカーを参照。必須論点と4リンクの一致を確認） |
| **AUD-5** | 進捗文書同期（研究結果・判断事項一覧） | Sonnet | AUD-0 | ✅ 完了（2026-07-20・研究結果を正本参照型へ整理しE3/V系/V4完了を同期、旧残作業・指標値ミラー候補0件。判断9項目を全て確定済みとし、図本文挿入をP2-DOC-2〔年末〕へ繰延） |
| **AUD-6** | allowlistコミット | Claude Code | AUD-0〜AUD-5 | ✅ 完了（2026-07-20・AUD関連12ファイルだけを明示allowlist化、unit 78件成功、正本3 SHAとroute SHA不変を確認。commitのみ、pushなし） |
| **AUD-7** | 実装終了時の研究結果充足性再監査 | Codex | AUD-0〜AUD-6,V4 | ✅ 完了（2026-07-20・Phase 1〜3の正本、Phase 3正本4 SHA、A3/B5、worst-off、S10、ECDF、E3/V3/V4を実測。unit 78件成功。RQ回答に必要な結果は充足し、追加run・追加実装は不要と判定。残作業はP2-DOC-2と卒論用静的図表の編集のみ） |

> **2026-07-20 AUD系（研究結果充足性監査の修正）追加：** [[研究結果充足性監査_20260720]] §9（Fable 5確定・手順0〜6）に基づき、AUD-0〜AUD-6のタスクカードを追加する。**全手順run不要**（既存8run・既存スクリプトの再利用のみ・追加run/seedは決定79・81が禁止）。実行順＝AUD-0（最優先）→AUD-1→AUD-2∥AUD-3（AUD-1後）→AUD-4（AUD-1,3後）→AUD-5（AUD-0後）→AUD-6（最後）。主結論ヌルは不変（決定105・106）。本セクションはグローバル決定番号を新規発番しない（既存決定47・79・81・107・108・123・129・134・136・141・144・146を参照のみ）。

> **2026-07-20 実装終了再監査（AUD-7）：** AUD-0〜AUD-6適用後の正本・補助結果・感度結果・図・HTML・統合テスト記録を実測し、**確定RQへ回答するための研究結果は得られる状態**と判定した。実装フェーズは終了可能。P2-DOC-2、図4-7-1・図5-4-1・図5-5-1、表5-6-1は既存結果を論文へ配置する編集タスクであり、結果不足や追加runの根拠にはしない。詳細は `開発メモ/研究結果充足性監査_20260720.md` §10。
...
## AUD系：研究結果充足性監査の修正（手順0〜6）
> **読む正本**：`開発メモ/研究結果充足性監査_20260720.md` §9（[[研究結果充足性監査_20260720]]・Fable 5確定済み・ローカルgitignore）。方針の新設・数値の再導出はしない。主結論はヌル確定（決定105・106・不変）。
> **共通制約（各カードに再掲・必須）**：追加run/seed禁止（決定79・81）／X-5文言境界＝方向主張・equivalence表現禁止（決定146）／撤回値（`decision128_baseline`旧値ほか§9.2 D-3対象）の引用禁止／指標値のハードコード禁止＝真実源はアーカイブ算出のみ（決定129・134）／コミットは明示allowlist・`git add .`禁止（決定136）／`git restore`・`git checkout -- output/`全面禁止（決定86）／判断記録（既存の決定番号付き記録）は不改変・前方ポインタ注記のみ（決定141）。
> **grep等の完了条件は実行時に再実測すること**（行番号・ヒット数を本カードにハードコードしない・決定134）。

### AUD-0｜撤回コピー隔離＋正本manifest作成（最優先）
- **担当**：mv・ハッシュ実測＝Claude Code／README・manifest文面＝Sonnet ／ **依存**：なし ／ **読む正本**：監査§3.7・§9.2 D-3・§9.3 手順0
- **作業内容**：
  1. `outputs/decision128_baseline/` を `outputs/retracted/decision128_pre_correction_20260713/` へ**ディレクトリごと**移動する（`replicate_metrics.csv`も分割せず一体で移す）。
  2. 移動先にREADMEを作成する。内容＝撤回済みである旨／誤値の由来（`e2004d7`のハードコード回帰＝`VERIFIED_COMPLETION_RATES`）／引用禁止／現行正本3ファイル（band_summary・符号表・E2のディスク実物）の絶対パス。
  3. 正本manifestを`06_研究結果/phase3/`配下に作成する。**指標値は転記しない**。内容＝3正本ファイルの絶対パス＋各SHA-256＋8run ID＋seed一覧＋route SHA `3d6977…8297`＋生成commit `f404837`のみ。
- **完了条件**：`outputs/decision128_baseline/`が存在しないことを実行時に確認する。retracted先にREADMEが存在する。manifestに数値リテラルが0件であることを実行時にgrepで確認する。移動は`git mv`ではなく通常`mv`とし、gitignore配下のため追跡へ影響しないことを実行時に確認する。

### AUD-1｜RQ一本化（固定ルート・方向中立・指標階層明文化）
- **担当**：Sonnet ／ **依存**：なし（他文書に先行して実施） ／ **読む正本**：監査§3.1・§9.3 手順1
- **対象**：`03_研究設計文書/共通設計/RQ・研究課題.md`
- **作業内容**：
  (a) 「動的ルート」の旧記述を固定ルート（確定仕様）へ修正する。
  (b) 研究目的の方向前提（「どれだけ改善するか」）を方向中立表現（「差が生じるか・本モデルで検出できるかを検証する」）へ修正する。
  (c) アプローチ図に「事前仮説であり結論ではない（決定47）」の注記を追加する。
  (d) 指標階層を明文化する：正式判定＝完了率のみ／記述報告＝完了時間ECDF（完了率と対）＋worst-off記述統計／従・限界＝渋滞・逃げ遅れ絶対数。
- **完了条件**：4点すべてが反映されている。方向主張・equivalence語が0件であることを実行時にgrepで確認する（決定146）。指標階層の記述が本文中に明記されている。

### AUD-2｜regime bimodality用語統一（現状文書のみ）
- **担当**：Sonnet ／ **依存**：なし（AUD-1と並行可） ／ **読む正本**：監査§3.4・§9.2 D-2・§9.3 手順2
- **対象**：現状文書のみ＝E3本文ドラフト・V3 HTML（`output/phase3.html`）・`06_研究結果/phase3/Phase3_研究結果.md`・`4年次本研究/CLAUDE.md`現状節。**判断記録（決定107本体）は改変せず前方ポインタ注記のみ**。
- **作業内容**：「regime bimodality」「二峰性」の断定表現を「強い初期条件感応性」「複数regimeの存在（の示唆）」へ統一する。層3比率・バックログ0等の具体観測記述は保持する。決定107本体には本手順で用語統一した旨の前方ポインタ注記のみ追加し、判断内容自体は書き換えない。
- **完了条件**：対象4文書で「二峰性」「bimodality」の断定表現が0件であることを実行時にgrepで確認する（**現状文書限定・判断記録は除外・決定144**）。観測事実の記述（層3比率・バックログ0等）が残存している。決定107本体が不改変であることを確認する。

### AUD-3｜worst-off記述統計CSV作成
- **担当**：実行＝Claude Code、文面＝Sonnet ／ **依存**：AUD-1 ／ **読む正本**：監査§3.2・§9.2 D-1・§9.3 手順3
- **作業内容**：
  ① `p3_evaluate_equity.py`を段3R全8runのarchived `vehicle_log.csv`へ適用する。
  ② A3・B5併記の記述統計CSVを1枚作成する（worst-off10%完了時間・percentiles・各runの到着率併記）。
  ③ **Δ判定列・符号列は作らない**（正式な定量A/B判定は完了率一本に限定・データ閲覧後の新規判定規則追加は決定79・102の事前登録原則が禁止）。
  ④ CSVヘッダとE3限界節に「車単位・到着者条件付き・B側バス乗客非含有・診断のみ・方向主張禁止」を明記する。
  ⑤ 渋滞は新規成果物を作らず、限界節に1段落を追記する。
- **完了条件**：CSVが1枚生成されている。判定列・符号列が存在しないことを実行時に列名で確認する。スコープ注記（車単位・到着者条件付き・非含有・診断のみ）がCSVヘッダとE3限界節の両方に存在する。追加SUMO run・追加seedが0件であることを確認する（決定79・81）。

### AUD-4｜限界節固定文言4点の追記
- **担当**：Sonnet ／ **依存**：AUD-1, AUD-3 ／ **読む正本**：監査§3.3・§3.5・§3.6・§3.9・§9.3 手順4
- **作業内容**：以下4点を固定文言として限界節に追記する。
  (a) 完了時間はECDF記述のみとし、母集団非可換のため新規統合指標は導入しない。
  (b) 15組合せは8run再利用であり独立標本でないため、信頼区間・有意差・同等性は主張しない。
  (c) 人単位会計は台数×2.3人の近似で分母3,231.5固定。二重計上はS10のraw102%で表面化しており、保守capは上限管理でありraw102%自体は保持する（決定123）。
  (d) Phase 2はteleport有効・単一run値のため「TraCI基盤確立・地域スクリーニング・Phase 3実装基盤」に限定する。
  RQ・E3・V3・研究結果の4文書で同一文言を参照する（コピペではなく参照・一致であることを確認する）。
- **完了条件**：4文言が対象4文書すべてに一致した形で存在することを実行時に確認する。X-5境界（方向主張・equivalence禁止・決定146）に抵触しないことを確認する。

### AUD-5｜進捗文書同期
- **担当**：Sonnet ／ **依存**：AUD-0（manifest） ／ **読む正本**：監査§3.8・§9.3 手順5
- **対象**：`06_研究結果/phase3/Phase3_研究結果.md`・`05_タスク管理/phase3/Phase3_判断事項一覧.md`
- **作業内容**：`Phase3_研究結果.md`をE3・V系完了・主結論ヌル・正本manifest（AUD-0で作成）参照へ更新する。`Phase3_判断事項一覧.md`をバス仕様確定済みへ更新する。数値はミラーせず正本パスを参照する。V4完了を記載する。図4-7-1・図4-8-1・図5-4-1の本文挿入は「P2-DOC-2（年末）で実施」と繰延を明記し、優先度A-6を「必須だが年末実施」へ再分類する。
- **完了条件**：両文書が現在状態を反映している。数値リテラルが最小（正本パス参照が主）であることを実行時に確認する。繰延の明記が存在する。

### AUD-6｜allowlistコミット
- **担当**：Claude Code ／ **依存**：AUD-0〜AUD-5すべて完了 ／ **読む正本**：決定136・決定86・§9.3 手順6
- **作業内容**：明示allowlistでコミットする。`git add .`は禁止。`scenario_b.rou.xml`は不可触。コミット前後でroute SHA `3d6977…8297`不変を実測する（決定136）。開発メモ（本監査ファイル含む）はgitignore対象のままローカル保持とし、コミット対象に含めない。
- **完了条件**：allowlist外のファイルがstagingに0件であることを実行時に`git status`で確認する。route SHAがコミット前後で不変であることを実行時にsha256sumで確認する。ユーザー承認があればpushする（承認がなければcommitのみに留める）。
