...
Phase 1（自家用車避難シナリオ）は **茨城県内40市区町村対応で完了**。  
...
│   │   ├── p1_city_road_network.py← 茨城県全40市区町村 道路NW取得
│   │   ├── city_scenario.py      ← 市別シナリオHTML生成（40市区町村）
...
│   │   ├── unified/              ← 茨城県40市区町村統合シミュレーション
│   │   ├── scenario_cities/      ← 市別シナリオHTML（40市区町村）
│   │   ├── scenario_v2/          ← 常総市単独シナリオ（参考）
│   │   ├── routes/               ← 実データ版時刻別ルートマップ
│   │   └── flood/                ← 浸水時系列マップ
│   │
│   ├── テスト結果_phase1_*.md    ← Phase 1 各段階のテスト記録
│   ├── テスト結果_t7到達不可分析_取手市_城里町.md
│   ├── テスト結果_対象外4市町村_除外理由分析.md
...
└── 05_タスク管理/                ← 進捗管理
    ...
    └── 茨城県内拡張_市区町村別タスク.md ← 40市区町村のA31a確認結果
```
...
[データ取得]                        [中間成果物]
  ...
  OSM 道路NW（osmnx）
  40市区町村                  →   {code}_road_network.graphml
...
## シミュレーション対象市区町村（40市区町村）

茨城県内44市区町村のうち、A31a 浸水想定区域ポリゴンが境界内に存在する40市区町村を対象。

**対象外（A31a 境界内0件 · 4市区町村）**
...
| 五霞町 | 利根川・渡良瀬川合流部。利根川系 A31a 茨城県版未収録（データ制約） |

> 五霞町は実際には高洪水リスク地域だが、利根川水系の浸水想定データが
> 茨城県 A31a-08 に含まれないため対象外（研究の限界として卒論に記載予定）。
...
- 市別シナリオ：茨城県内**40市区町村** の HTML ページを生成（A31a 段階的閉鎖）
