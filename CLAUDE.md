...
## 現在フェーズ（最終更新：2026/05/11）

**Phase 1 — 実データ版 閉鎖タイムライン構造バグ修正完了・市区町村拡張41市区町村**

- `CLOSURE_SOURCE = "kml_a31a"` に変更（旧: suiboumap_hydrograph）
- `e1_load_flood_data.py` の `main()` で A31a を N03 常総市境界クリップ（1495ポリゴン）
- 実データ版の累積閉鎖: t0=168本 → t1=764本 の単調増加を実現
- A31a 要調査7市町村の調査完了 → 4市町村追加（古河市・坂東市・八千代町・境町）
- 五霞町（08542）追加: A31a 08_20 の境界±500mバッファで501ポリゴン取得・シナリオ生成完了
  - 五霞町は河川行政境界の精度差により境界内0件→ バッファ拡張で対応
  - 閉鎖エッジ数はすべて0（浸水想定域が道路NWと非交差）
- シナリオ生成対象: **41市区町村**（40 → 41）
- 対象外（A31a境界内0件）: 鹿嶋市・神栖市・東海村（計3市町村）
