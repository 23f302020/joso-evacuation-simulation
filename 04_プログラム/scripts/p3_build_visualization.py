...
def main() -> None:
    ...
    domain_min, domain_max = -5.0, 25.0

    def position(value: float) -> float:
        return (value - domain_min) / (domain_max - domain_min) * 100.0

    raw_min = band["raw_delta_min_percentage_points"]
    raw_max = band["raw_delta_max_percentage_points"]
    cons_min = band["conservative_delta_min_percentage_points"]
    cons_max = band["conservative_delta_max_percentage_points"]
    a_values = [value * 100.0 for value in band["a_completion_rate_values"]]
    raw_counts = band["raw_sign_counts"]
    cons_counts = band["conservative_sign_counts"]
    s10_raw_counts = s10["raw_sign_counts"]
    s10_cons_counts = s10["conservative_sign_counts"]
...
<section><h2>A/B完了率差の不確実性帯</h2><div class="plots"><div class="plot"><div class="label">差（B−A、%pt）: 赤破線が0</div><div class="axis"><div class="zero" style="left:{position(0):.1f}%"></div><div class="band" style="left:{position(raw_min):.1f}%;width:{position(raw_max)-position(raw_min):.1f}%"></div><div class="band cons" style="left:{position(cons_min):.1f}%;width:{position(cons_max)-position(cons_min):.1f}%"></div></div><div class="legend"><span><i class="sw" style="background:#16827a"></i>raw {raw_min:+.2f}〜{raw_max:+.2f}（正{raw_counts['positive']}・負{raw_counts['negative']}）</span><span><i class="sw" style="background:#4574a8"></i>保守 {cons_min:+.2f}〜{cons_max:+.2f}（正{cons_counts['positive']}・負{cons_counts['negative']}）</span></div></div>
<div class="plot"><div class="label">A側完了率（%）: seed別</div><div class="axis" id="bimodal"><span class="dot" style="left:5%;top:74%"></span><span class="dot" style="left:88%;top:42%"></span><span class="dot" style="left:93%;top:24%"></span></div><div class="legend">{' / '.join(f'{value:.2f}%' for value in a_values)} — 早期広域ロックと高完了regime</div></div></div></section>
<section><h2>バス10台感度（増車のみ・削減54台固定）</h2><div class="plot"><div class="label">rawは二重計上バイアスを含むため、保守系列を併記</div><div class="legend"><span><i class="sw" style="background:#16827a"></i>raw {s10['raw_completion_rate_min_percent']:.2f}〜{s10['raw_completion_rate_max_percent']:.2f}%（正{s10_raw_counts['positive']}・負{s10_raw_counts['negative']}）</span><span><i class="sw" style="background:#4574a8"></i>保守 {s10['conservative_completion_rate_min_percent']:.2f}〜{s10['conservative_completion_rate_max_percent']:.2f}%（正{s10_cons_counts['positive']}・負{s10_cons_counts['negative']}）</span></div><p>S10#4はraw 102.01%を保持し、保守97.15%を併記する。いずれの系列も符号は非一貫で、方向差を示さない。</p></div></section>
