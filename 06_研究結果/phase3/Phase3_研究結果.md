...
図4-7-1、図4-8-1、図5-4-1の卒論本文への最終挿入は、**P2-DOC-2（年末）で実施**する。図のデータ・既存実体は保持し、現時点では新規runや新規判定規則を追加しない。優先度A-6は「**必須だが年末実施**」として管理する。

Phase 3の実装・評価として残る作業はなく、今後はP2-DOC-2と論文編集上の図挿入を行う。
