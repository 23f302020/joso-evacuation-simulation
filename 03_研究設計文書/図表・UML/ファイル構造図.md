...
> 作成日：2026/04/28  
> 最終更新：2026/07/20（Phase 3スクリプト・正本評価成果物を追記）
> 対象：Phase 1〜3 実装に関わるファイル・ディレクトリ構成
...
│   ├── p2_traci_common.py           ✅          A/B共通TraCI実行処理
│   ├── p2_traci_bus.py              ✅          固定ルートバス混在シナリオ実行
│   │
│   │── 【Phase 3 評価・成果物】
│   ├── p3_bus_accounting.py         ✅          救出走行・バス輸送会計
│   ├── p3_bus_scenario.py           ✅          シナリオB route/config生成
│   ├── p3_correct_bus_passenger_accounting.py ✅ 乗車中孤立を含む補正会計
│   ├── p3_phase3r_e1_bands.py       ✅          A3/B5反復評価・raw/保守帯
│   ├── p3_s10_sensitivity_metrics.py ✅         バス10台感度評価
│   ├── p3_fig48_completion_time_ecdf.py ✅      完了時間ECDF生成
│   ├── p3_build_visualization.py    ✅          Phase 3可視化生成
│   └── p3_build_phase3_excel.mjs    ✅          Phase 3 Excel生成
│
├── data/                                        入力データ（読み取り専用）
│   ├── flood_kml/
...
│
├── テスト結果/                      ✅          テスト記録（フェーズ別）
│   ├── phase1/                                 Phase 1テスト記録（README.mdで索引化）
...
### 1-1. Phase 3 常総市正本成果物

```text
output/sumo/regions/08211/
├── derived/
│   └── scenario_b_rescue_reduction.csv
├── results/
│   ├── a1/a2/a3・b1〜b5 seed別gate JSON
│   ├── scenario_b_bus_corrected_accounting.json
│   ├── scenario_b_bus_summary.json
│   └── archive_runs/                         正本候補・履歴run
└── evaluation/
    ├── phase3r_e1_replicate_metrics.csv      正本8run
    ├── phase3r_e1_15_combination_signs.csv   A3×B5差
    ├── phase3r_e1_band_summary.json           raw/保守要約
    ├── phase3_ab_comparison.csv
    ├── phase3_s10_replicate_metrics.csv
    ├── phase3_s10_15_combination_signs.csv
    └── phase3_s10_band_summary.json

06_研究結果/phase3/figures/
└── fig4-8-1_completion_time_ecdf.svg
```

> `archive_runs/`には途中停止・履歴runも含まれるため、フォルダ名だけで正本判定しない。評価CSVのrun IDとseed別gateを経由して追跡する。
