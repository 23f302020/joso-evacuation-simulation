...
> 位置づけ：判断不要の下準備文書。主要指標定義は `Phase2_実装前仕様.md` に反映済み。
...
全量実行でログ量が大きくなる場合は、SQLiteへの移行を検討する。
...
| 項目 | 判断が必要な内容 |
|---|---|
| 逃げ遅れ定義 | 採用済み：未到着かつ到達不能または長時間停止 |
| 時間軸 | 採用済み：t0〜t7を6時間へ圧縮 |
| 保存形式 | 採用済み：初回CSV、重くなったらSQLite |
| 車両単位 | 採用済み：小規模は1メッシュ1台、1/10以降は世帯換算 |
